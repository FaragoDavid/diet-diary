...
export default function MealsPage({ uid }: { uid: string }) {
  ...
  if (error) {
    return (
      <div className="alert alert-error">
        <span>Failed to load days: {error}</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {days.length === 0 ? (
        <div className="text-center py-12 text-base-content/50">No days recorded yet. Create your first one!</div>
      ) : (
      ...
      <div className="card-body p-4 flex-row items-center justify-between">
        <div>
          ...
          <div className="text-sm text-base-content/60 mt-1">
            {round(totals.calories)} cal · {round(totals.carbs)}g carbs · {round(totals.fat)}g fat
            {dishCount > 0 && (
              <span>
                {' '}
                · {day.meals.length} meals · {dishCount} dishes
              </span>
            )}
          </div>
        </div>
